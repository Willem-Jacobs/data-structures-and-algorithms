...
const getBaseStatGreaterThan = (arr, minBaseStat) => {
  // Solution code here...
  const newArray = arr.filter((item) => item.baseStat > minBaseStat);
  return newArray;
};
...
const getStatName = (arr, minBaseStat) =>
  arr.filter((stat) => stat.baseStat > minBaseStat).map((obj) => obj.stat.name);
...
const getCharactersWithoutChildren = (arr) => {
  // Solution code here...
  const newArray = arr.filter((item) => ("children" in item ? null : item));
  return newArray;
};
...
const evenOddNumericValues = (arr) => {
  // Solution code here...
  const firstArray = arr.filter((item) => typeof item === "number");
  const secondArray = firstArray.map((item) => (item % 2 ? "odd" : "even"));
  return secondArray;
};
